...
namespace ProjectGame.HotFix.Gameplay.Player.Sync
{
    ...
    public sealed class PlayerSyncController : NetworkBehaviour, IPlayerSyncEndpoint, IPoolable
    {
        ...
        /// <summary>
        /// 当前玩家端点的纯 C# 模拟游标；统一保存 Tick 编号并计算单步时长。
        /// 推进节奏来自通用 NetworkSimulationClock，Owner 校正仍可独立重置本地预测游标。
        /// </summary>
        private PlayerSimulationClock _simulationClock;

        /// <summary>当前网络会话共享的通用 Clock、Transport 与统计服务。</summary>
        private GameplayNetworkRuntime _networkRuntime;
        ...
        public override void OnNetworkSpawn()
        {
            ...
            _networkRuntime = GameNetworkRuntime.Gameplay;
            _transport = GameNetworkRuntime.PlayerSync;

            if (_networkRuntime == null || !_networkRuntime.IsInitialized ||
                _transport == null || !_transport.IsInitialized)
            {
                Debug.LogError($"[{nameof(PlayerSyncController)}] Gameplay 网络运行时或 PlayerSync 协议尚未初始化 ");
                return;
            }
        ...
        private void HandleNetworkTick(uint sessionTick)
        {
            ...
            if (IsServer)
            {
                _simulationClock.SynchronizeToSessionTick(sessionTick);
                RunServerTick(sessionTick);
            }
            else if (IsOwner)
            {
                uint predictionTick = _simulationClock.AdvanceOneTick();
                RunOwnerPredictionTick(predictionTick);
            }
        }
        ...
        public void ResetAfterWarp(Vector3 position, Quaternion rotation)
        {
            ...
            state.PivotBoostTimeRemaining = 0f;
            state.PivotBoostSpeedBonus = 0f;
        ...
        private void ValidateTickRate()
        {
            ...
            int networkTickRate = _networkRuntime.Clock.TickRate;
            if (networkTickRate != _config.SimulationTickRate)
            {
                throw new InvalidOperationException(
                    $"玩家模拟 TickRate={_config.SimulationTickRate}，Gameplay 网络 TickRate={networkTickRate}，两者必须一致。");
            }
        }
