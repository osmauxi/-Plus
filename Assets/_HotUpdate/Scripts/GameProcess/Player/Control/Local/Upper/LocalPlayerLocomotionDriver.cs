...
namespace ProjectGame.HotFix.Gameplay.Player.Movement
{
    ...
    public sealed class LocalPlayerLocomotionDriver : NetworkBehaviour
    {
        ...
        /// <summary>
        /// 从屏幕指针发射射线，与玩家当前高度的水平面求交 
        /// 返回用于 Simulation 的单位方向，同时输出供 Camera LookAhead 使用的世界坐标 
        /// </summary>
        private Vector3 ResolveAim(
            Vector2 pointerPosition,
            out Vector3 aimWorldPosition)
        {
            Ray ray = _worldCamera.ScreenPointToRay(pointerPosition);
            Vector3 fallbackDirection = Flatten(transform.forward).normalized;
            ...
            {
                aimWorldPosition = transform.position + fallbackDirection * 10f;
                return fallbackDirection;
            }

            aimWorldPosition = ray.GetPoint(distance);
            Vector3 aimDirection = Flatten(aimWorldPosition - transform.position);
