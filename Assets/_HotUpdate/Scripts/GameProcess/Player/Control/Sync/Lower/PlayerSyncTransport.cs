...
namespace ProjectGame.HotFix.Gameplay.Player.Sync
{
    ...
    ///
    /// 本类保留 Player 消息格式、对象路由和 Baseline 解析；实际消息注册、发送与 Delivery 映射
    /// 委托给通用 NetworkMessageTransport。它不负责预测、模拟、插值和回滚。
    /// </summary>
    public sealed class PlayerSyncTransport
    {
        ...
        /// <summary>通用 Gameplay Named Message 传输服务；本类只保留 Player 专用协议与对象路由。</summary>
        ...
        private bool _registeredInputHandler;
        private bool _registeredSnapshotHandler;

        /// <summary>传输层是否可以开始注册端点和收发消息 </summary>
        public bool IsInitialized => _initialized;

        /// <summary>在通用 Gameplay Transport 上创建玩家同步协议适配层。</summary>
        public PlayerSyncTransport(NetworkMessageTransport transport)
        {
        ...
        public void Initialize()
        {
            ...
            {
                _transport.RegisterHandler(InputMessageName, OnInputMessageReceived);
                _registeredInputHandler = true;
            }

            if (_transport.IsClient)
            {
                _transport.RegisterHandler(SnapshotMessageName, OnSnapshotMessageReceived);
                _registeredSnapshotHandler = true;
            }
        ...
        public void Shutdown()
        {
            ...
            if (_transport.IsInitialized && _registeredInputHandler)
                _transport.UnregisterHandler(InputMessageName);

            if (_transport.IsInitialized && _registeredSnapshotHandler)
                _transport.UnregisterHandler(SnapshotMessageName);

            _registeredInputHandler = false;
            _registeredSnapshotHandler = false;
        ...
        public void SendSnapshot(ulong targetClientId, ulong networkObjectId, in PlayerSnapshotPacket snapshot)
        {
            ...
            NetworkDeliveryClass delivery = snapshot.IsFull
                ? NetworkDeliveryClass.FullSnapshot
                : NetworkDeliveryClass.DeltaSnapshot;
